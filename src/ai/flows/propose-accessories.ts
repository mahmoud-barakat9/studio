...
1.  **Main Axis (Tube):** Required. One for each opening. The length should be the codeLength of the opening + 10cm. Sum the total length required for all openings and provide one final number in 'meter' unit.
...
3.  **Channels (Majari):** Required if 'hasAccessories' is true for an opening. Two for each such opening. The length of each channel is opening height + 5cm. Sum the total length required and provide result in 'meter'. If height is not available, do not suggest channels.
...
7.  **Remote Control:** Optional. If motors are included, suggest one remote per 3 motors.
8.  **Hangers (Hamalat):** Required if 'hasInstallation' is true. Suggest 2 units per opening.
9.  **Bottom Bar (Barra):** Required. One for each opening. Length is codeLength. Sum total length and provide in 'meter'.
