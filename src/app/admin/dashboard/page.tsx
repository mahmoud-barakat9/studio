...
export default async function AdminDashboardPage() {
  ...
  const avgPurchasePrices = purchases.reduce(
    (acc, p) => {
  ...
  const materialCosts = Object.fromEntries(
    Object.entries(avgPurchasePrices).map(
      ([name, { totalCost, totalQuantity }]) => [
        name,
  ...
  const monthlyTotalArea = thisMonthOrders.reduce(
    (sum, o) => sum + o.totalArea,
    0,
  );
  const monthlySales = thisMonthOrders.reduce(
    (sum, o) => sum + o.totalCost + (o.deliveryCost || 0),
    0,
  );
  ...
  const monthlyOrdersComparison = getComparison(
    monthlyOrdersCount,
  ...
  const monthlyTrendsData = Array.from({ length: 12 }, (_, i) => {
    ...
    return {
      ...
      totalArea: parseFloat(
        monthOrders.reduce((sum, o) => sum + o.totalArea, 0).toFixed(2),
      ),
    };
  });

  // Top materials by totalArea
  const materialsData = orders.reduce(
    (acc, order) => {
  ...
  const nextMonthOrdersForecast = Math.round(
    last3Months.reduce((sum, m) => sum + m.orders, 0) /
      (last3Months.length || 1),
  );
  const nextMonthAreaForecast = parseFloat(
    (
      last3Months.reduce((sum, m) => sum + m.totalArea, 0) /
      (last3Months.length || 1)
    ).toFixed(1),
  );

  const peakPeriods = [...monthlyTrendsData]
  ...
  const getUserName = (userId: string) =>
    users.find((u) => u.id === userId)?.name || "غير معروف";
    ...
    <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
      <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
        <KpiCard
          ...
          value={`${kpiData.todaysOrdersCount}`}
          comparisonText=""
          isPositive={null}
          Icon={CalendarDays}
        />
        <KpiCard
          ...
          value={`${kpiData.monthlyOrdersCount}`}
          comparisonText={kpiData.monthlyOrdersComparison.text}
          isPositive={kpiData.monthlyOrdersComparison.isPositive}
          Icon={ClipboardList}
        />
        <KpiCard
          title="قيمة المبيعات الشهرية"
          value={`$${kpiData.monthlySales?.toFixed(2)}`}
          comparisonText={kpiData.monthlySalesComparison.text}
          isPositive={kpiData.monthlySalesComparison.isPositive}
          Icon={DollarSign}
        />
        <KpiCard
          title="الأرباح الشهرية"
          value={`$${kpiData.monthlyProfit?.toFixed(2)}`}
          comparisonText={kpiData.monthlyProfitComparison.text}
          isPositive={kpiData.monthlyProfitComparison.isPositive}
          Icon={HandCoins}
        />
      </div>
      <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
        <KpiCard
          title="إجمالي المتر المربع (شهري)"
          value={`${kpiData.monthlyTotalArea?.toFixed(2)} م²`}
          comparisonText=""
          isPositive={null}
          Icon={Ruler}
        />
        <KpiCard
          ...
          value={`${kpiData.inProductionCount}`}
          comparisonText=""
          isPositive={null}
          Icon={Package}
        />
        <KpiCard
          ...
          value={`${kpiData.lateOrdersCount}`}
          comparisonText="تحتاج إلى انتباه فوري"
          isPositive={false}
          Icon={AlertTriangle}
          className={
            kpiData.lateOrdersCount > 0
              ? "border-destructive text-destructive shadow-lg hover:shadow-xl transition-shadow"
              : ""
          }
        />
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-4 md:gap-8">
        ...
        />
        <Card className="shadow-lg hover:shadow-xl transition-shadow xl:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <BrainCircuit /> توقعات وتحليلات ذكية
            </CardTitle>
            <CardDescription>
              تقديرات للشهر القادم بناءً على بيانات آخر 3 أشهر.
            </CardDescription>
          </CardHeader>
            ...
            <div className="flex items-start gap-3 rounded-lg border bg-muted/40 p-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium">
                  توقع عدد الطلبات الشهر المقبل
                </p>
                <p className="text-2xl font-bold">
                  ~{forecasts.nextMonthOrdersForecast} طلب
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border bg-muted/40 p-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-2xl font-bold">
                  ≈ {forecasts.nextMonthAreaForecast} م²
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border bg-muted/40 p-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium">
                  فترات الذروة (أعلى 3 أشهر)
                </p>
                <p className="text-lg font-bold">
                  {forecasts.peakPeriods?.join("، ")}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="col-span-full shadow-lg hover:shadow-xl transition-shadow">
          <CardHeader className="flex flex-row items-center">
            <div className="grid gap-2">
              <CardTitle className="flex items-center gap-2">
                <ListChecks /> الطلبات الحرجة
              </CardTitle>
              <CardDescription>
                الطلبات المتأخرة التي تحتاج إلى متابعة فورية.
              </CardDescription>
            </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-8">
        <Card className="shadow-lg hover:shadow-xl transition-shadow">
          <CardHeader className="flex flex-row items-center">
            ...
            <Button asChild size="sm" className="mr-auto gap-1">
              <Link href="/admin/users">عرض الكل</Link>
            </Button>
          </CardHeader>
          <CardContent>
            <Table>
